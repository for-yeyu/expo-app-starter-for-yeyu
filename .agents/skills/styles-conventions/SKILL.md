...
3. Keep Tailwind directives in `src/styles/global.css`.
4. `index.css` must import `global.css`.
5. New global CSS files must be separate `.css` files imported by `index.css`.
6. Prefer NativeWind `className` for screen and component styling.
7. Keep one-off navigator/runtime colors close to the navigator configuration.
8. Do not introduce a style token module for one-off values.
